...
| **3** | **H_PRECISION_600** | New physics-residual detector, target 600 | **A3 vs A2** (precision) | Benchmark against **H5 (33.34)**. Follow the curve to see if the new precision features allow us to beat the old 600 peak. | queued |
| **4** | **CONS_PRECISION_500_sw035** | New detector 500 + sw_max 0.35 | **A2 x A3** (consolidation) | Lock in the precision gains at 500 with our best known footage cut. | queued |
| **5** | **TBD** | TBD | **A3** (reactive) | Final flex slot. | queued |

### Probe 1 & 2 Results: The Read
*   `H_PRECISION_400`: **30.13** (vs old 29.91) -> **+0.22 gain**
*   `H_PRECISION_500`: **31.86** (vs old 31.50) -> **+0.36 gain**

The new physics-residual features **did** improve precision. We are catching more true honeypots within the same target nets. However, the gains are modest. We haven't completely solved the problem (we aren't hitting 34+ at 500 yet), meaning some true honeypots are still slipping past our new features, or the features need re-weighting. But the direction is correct. We will use the remaining 3 slots to push the curve to 600 and test the `CONS` stack.
...
**Scoreboard** (auto, updated 2026-06-27 09:12) — best **34.52** (CONS); leader target 37.
