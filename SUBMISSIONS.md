...
| **5** | **H_PRECISION_700** | New physics-residual detector, target 700 | **A3 vs A2** (precision) | Benchmark against **H6 (34.43)**. Complete the curve comparison to see if the new detector creates a new absolute peak. | queued |

### Probe Results & The Read
...
**Analysis:**
1.  **Diminishing Returns on Precision:** The precision features *are* working, but the margin of victory is shrinking as the net gets wider (+0.36 at 500, but only +0.06 at 600). This means the new features successfully pushed some honeypots up from the >700 ranks into the 400-600 ranks, but not enough to change the fundamental shape of the curve.
2.  **The Flat CONS:** The fact that `CONS` scored exactly 31.86 means the tighter footage cut (`sw_max=0.35`) provided *zero* additional benefit on top of the 500 honeypot veto. The marginal wells that `sw_max=0.35` would have cleaned up were already vetoed by the honeypot detector. They are stepping on each other's toes.

**Next Steps (Slot 5):**
For the final slot of Day 2, we will run `H_PRECISION_700`. This completes the curve comparison. If it beats the old `H6` (34.43), we have a new absolute peak. If it falls short, it means our precision features are too heavily weighted and are now falsely catching real wells at the top of the curve.
...
**Scoreboard** (auto, updated 2026-06-27 11:01) — best **34.52** (CONS); leader target 37.
